  ...
  return (
    <div>
      <div className="tabs">
        ...
        {/* {content && content.map((_, index) => <Tab num={index} key={index} activeTab={activeTab} onClick={setActiveTab} />)}
        <Tab num={3} activeTab={activeTab} onClick={setActiveTab} /> */}
      </div>

      {activeTab <= 2 ? <TabContent item={content.at(activeTab)} key={content.at(activeTab).summary}/> : <DifferentContent /> }
      {/* // ! Eger key propertiesinden istifade etmesek bir componentdeki deyisikliyi diger component goturecek sehifeler arasi kecid edende render olmuyacaq */}
    </div>
  );
}

function Tab({ num, activeTab, onClick }) {
  return <button className={activeTab === num ? "tab active" : "tab"} onClick={() => onClick(num)} >Tab {num + 1}</button>
}
...
function TabContent({ item }) {
  ...
  function handleTripleInc() {
    setLikes((like)=>like + 1);     // ! 0 + 1 = 1
    setLikes((like)=>like + 1);     // ! 1 + 1 = 2
    setLikes((like)=>like + 1);     // ! 2 + 1 = 3
  }
    ...
    <div className="tab-content">
      ...
      <div className="tab-actions">
        ...
        <div className="hearts-counter">
          <span>{likes} ❤️</span>
          <button onClick={handleInc} onDoubleClick={handleOneInc}>+</button>
